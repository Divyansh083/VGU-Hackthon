...
authRouter.post("/user/forgotpassword", async (req, res) => {
    try {
        ...
        if (user) {
            ...
            res.status(400).json({
                message:
                    "user with this email is not exists please signup first",
            });
        }
    } catch (e) {
...
// authentication for shg

// shg_name string
// shg_id string
// documents array

authRouter.post("/shg/auth", auth, async (req, res) => {
    try {
        console.log("hi");
        ...
        user.shg_name = req.body.shg_name;
        user.shg_id = req.body.shg_id;
        user.shg_upi = req.body.shg_upi;
        user.shg_contact = req.body.shg_contact;
        user.shg_documents = req.body.shg_documents;
        user.seller_registeration_application_submitted_at = Date.now();
        user = await user.save();

        nodemailer.SendEmailVerification(
            user.useremail,
            `
        Thankyou for joining us we will verify your account within 24 hours
    `
        );
        res.status(200).json({
            msg: "succesfully applied for hosting the products on our site",
        });
    } catch (e) {
        console.log(e.message);
